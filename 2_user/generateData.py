...
def generateWorkerID(index):
    return '1818600%02d' % index

# generate dept data
print("\ngenerate department data")
department_list = ["公关","开发","运营","产品"]
for index,dept in enumerate(department_list):
    id = index
    name = dept
    chairman_id = generateWorkerID(index)
    print("({},\'{}\',{}),".format(id,name,chairman_id))
...
workers = []
for i in range(10):
    id = generateWorkerID(i)
    name = random_name()
    age = random.randint(23,40)
    salary = random.randint(20000,999999)
    if i < len(department_list): # 安排部门负责人
        dept_id = i
    else:
        dept_id = random.randint(0,len(department_list)-1)
    workers.append({'id':id,'name':name,'dept_id':dept_id,'age':age,'salary':salary})
    print("({},\'{}\',{},{},{}),".format(id,name,age,salary,dept_id))



# generate project data
print("\ngenerate project data:")
project_list = ["Office","SQL","VSCode","Visual Studio","Edge"]
projects = []
id = 0
for project in project_list :
    name = project
    dept_id = random.randint(0,len(department_list)-1)
    projects.append({'id':id,'name':name,'dept_id':dept_id})
    print("({},\'{}\',{}),".format(id,name,dept_id))
    id += 1
